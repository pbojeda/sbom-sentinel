...
export function loadConfig(
  ...
  // Step 9 — Build storage configs (STORAGE_PROVIDER may be comma-separated, e.g. "ibm-cos,google-drive")
  const rawProviders = (process.env['STORAGE_PROVIDER'] ?? '').split(',').map((s) => s.trim()).filter(Boolean);
  const storageConfigs: StorageConfig[] = [];

  for (const storageProvider of rawProviders) {
